...
def _prepare_dataframes(jira_file, defects_file, raid_file):
    """
    Loads raw CSV data and performs initial cleaning and pre-processing
    that is common across multiple metric calculations.
    Returns pre-processed DataFrames.
    """
    try:
        if isinstance(jira_file, str) and jira_file.endswith('.csv'): # Handle path for mock data
            df_jira = pd.read_csv(jira_file)
        else: # Handle UploadedFile
            df_jira = pd.read_csv(jira_file)

        if isinstance(defects_file, str) and defects_file.endswith('.csv'):
            df_defects = pd.read_csv(defects_file)
        else:
            df_defects = pd.read_csv(defects_file)
        
        if isinstance(raid_file, str) and raid_file.endswith('.csv'):
    ...
    if not all(col in df_jira.columns for col in required_jira_cols):
        ...
        raise ValueError(f"JIRA data is missing required columns: {', '.join(missing_cols)}.")
    ...
    df_jira.sort_values(by='SprintNumeric', inplace=True, ignore_index=True)

    # --- Defects Data Cleaning ---
    ...
    if not all(col in df_defects.columns for col in required_defects_cols):
        ...
        raise ValueError(f"Defects data is missing required columns: {', '.join(missing_cols)}.")
    ...
    if not all(col in df_raid.columns for col in required_raid_cols):
        ...
        raise ValueError(f"RAID data is missing required columns: {', '.join(missing_cols)}.")
    
    df_raid['DueDate'] = pd.to_datetime(df_raid['TargetDate'], errors='coerce')

    return df_jira, df_defects, df_raid

def _get_unique_sorted_sprints(df_jira: pd.DataFrame) -> pd.DataFrame:
    """Helper to get unique sorted sprints."""
    unique_sprints_df = df_jira[['SprintID', 'SprintNumeric']].drop_duplicates().sort_values('SprintNumeric', ignore_index=True)
    return unique_sprints_df

def calculate_velocity_trend(df_jira: pd.DataFrame) -> List[Dict[str, Any]]:
    """
    Calculates the velocity trend (completed story points per sprint).
    """
    unique_sprints_df = _get_unique_sorted_sprints(df_jira)
    ...
    return velocity_trend_df.to_dict('records')

def calculate_sprint_completion(df_jira: pd.DataFrame) -> List[Dict[str, Any]]:
    """
    Calculates sprint goal completion (committed vs. completed points).
    """
    unique_sprints_df = _get_unique_sorted_sprints(df_jira)
    df_jira_stories = df_jira[df_jira['Type'] == 'Story'] 
    df_jira_done = df_jira_stories[df_jira_stories['Status'] == 'Done']
    ...
    velocity_trend_data = calculate_velocity_trend(df_jira) # Re-use or get raw
    velocity_trend_df = pd.DataFrame(velocity_trend_data)
    ...
    return completion_trend.to_dict('records')

def calculate_capacity_utilization(df_jira: pd.DataFrame, num_sprints: int = 5) -> List[Dict[str, Any]]:
    """
    Calculates capacity utilization for the last N sprints.
    """
    latest_sprints_numeric = sorted(df_jira['SprintNumeric'].unique(), reverse=True)[:num_sprints]
    ...
    return capacity_data_df.to_dict('records')

def calculate_defect_density(df_jira: pd.DataFrame, df_defects: pd.DataFrame) -> List[Dict[str, Any]]:
    """
    Calculates defect density (defects per sprint vs. stories).
    """
    unique_sprints_df = _get_unique_sorted_sprints(df_jira)
    df_jira_stories = df_jira[df_jira['Type'] == 'Story'] 
    ...
    return density_data_df[['RaisedIn', 'SprintNumeric', 'DefectCount', 'StoryCount']].to_dict('records')

def calculate_defect_stage_distribution(df_defects: pd.DataFrame) -> List[Dict[str, Any]]:
    """
    Calculates the distribution of open defects by phase.
    """
    stage_data_df = df_defects[df_defects['Status'] == 'Open'].groupby('Phase').size().reset_index(name='Count')
    return stage_data_df.to_dict('records')

def get_raid_summary(df_raid: pd.DataFrame) -> List[Dict[str, Any]]:
    """
    Generates a summary of open RAID (Risks, Assumptions, Issues, Dependencies) items.
    """
    ...
    def get_raid_status_display(row):
        if 'Open' in str(row['Status']) or 'New' in str(row['Status']): 
            ...
            return '🚨 Overdue'
        return '🟢 Active' # This case might not be hit if we filter for "Open" only, but kept for robustness

    if not open_raid_detailed_df.empty:
        ...
        raid_grouped_status['Status'] = '✅' # Default to healthy
        raid_grouped_status.loc[raid_grouped_status['Overdue'] > 0, 'Status'] = '🚨'
        raid_grouped_status.loc[raid_grouped_status['Open'] > 0, 'Status'] = '⚠️' # Warning if just open

        return raid_grouped_status[['Type', 'Status', 'Total', 'Open', 'Overdue']].to_dict('records') # Added Overdue
    else:
        return [] # Return empty list if no open RAID items

# The main function that app.py will call to get all processed dataframes
def load_and_preprocess_raw_data(jira_file, defects_file, raid_file):
    """
    Loads and preprocesses raw CSV data into pandas DataFrames.
    This function returns the raw, preprocessed DataFrames,
    which can then be passed to individual metric calculation functions.
    """
    try:
        df_jira, df_defects, df_raid = _prepare_dataframes(jira_file, defects_file, raid_file)
        return {
            "df_jira": df_jira,
